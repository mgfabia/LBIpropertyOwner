...
- `silver_parcels` — one row per parcel (19,958 rows), keyed on `pams_pin`. Schema DDL in `sql/silver_parcels.sql`. Merges `bronze_tax_board` + `bronze_cadastral` via full outer join (18,278 both sources, 707 tax-board-only, 973 cadastral-only). Includes PostGIS geometry, normalized mailing address, and derived ownership columns (`is_absentee`, `mailing_state`, `is_nj_resident`). Also carries owner-name columns `owner_names` (grantees = current owner(s)) and `owner_grantors` (prior owner(s)), joined from `bronze_clerk_deeds` by the parcel's deed of record — 15,608 parcels have ≥1 owner name. `NULL` = no deed matched; `{}` (empty) = deed found but no name parsed; filter with `array_length(owner_names, 1) >= 1` for "has a name". Tax board wins for assessment/building data; cadastral wins for spatial/classification data. Populated by `build_silver_parcels.py` (truncate-and-rebuild loads rows via REST, then `populate_geometry` and `populate_owner_names` run raw SQL via the pooler); rebuild after reloading bronze data. Joins to SR1A sales via `silver_sr1a_pin_crosswalk.resolved_pams_pin`.
